...
export const getEmpleado = async (req, res) => {
    try{
    const {rows} = await pool.query(`SELECT emp_identificacion, emp_p_nombre, emp_p_apellido, carg_nombre
...
export const getProjectsProgress = async (req, res) => {
    try{
        const {rows} = await pool.query("SELECT pro_id, pro_nombre, pro_fecha_fin" +
...
export const getProjects = async (req, res) => {
    try {
        ...
        const {rows} = await pool.query("SELECT p.pro_id, p.pro_nombre "+
                                    "FROM proyecto p,estatus e,pro_estatus pe "+
                                    "WHERE p.pro_id = pe.proes_pro_id AND e.est_id = pe.proes_est_id "+
                                    "AND e.est_nombre = $1 AND e.est_tipo = 'Proyecto'",[status]);
...
export const getMineralsConfiguration = async (req,res) => {
    try {
        const {rows} = await pool.query(`SELECT min_id, min_nombre, COUNT(DISTINCT e.et_id) as "numero etapas", COUNT(a.act_id) as "numero actividades"
            ...
            if(!rows.length){
                return res.status(200).json({message: `No hay minerales en el sistema`});
            }
            return res.status(200).json(rows);                        
    }catch(error){
...
//la infomacion es la que se quiere
    const {rows} = await pool.query(`select min_id, min_nombre, et_nombre, et_num_etapa, act.act_nombre, act.act_prioridad
                FROM mineral, etapa, (select fk_et_id, act_nombre, act_prioridad
					                     from actividad) as act
                      WHERE min_id = $1
                      AND min_id = fk_min_id
                      AND et_id = act.fk_et_id`,[id]);
...
export const getMineralName = async (req, res)=>{
    ...
    const {rows} = await pool.query(`SELECT min_id, min_nombre, COUNT(DISTINCT e.et_id) as "numero etapas", COUNT(a.act_id) as "numero actividades"
                                        ...
                                        GROUP BY min_id, min_nombre`,[name]);
        if(!rows.length){
          return res.status(200).json({message: `No hay mineral por nombre: ${name}`});
                }
          return res.status(200).json(rows);        
    }catch(error){
        return res.status(500).json(error); 
